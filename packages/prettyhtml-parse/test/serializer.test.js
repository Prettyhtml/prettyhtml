...
exports[
  "Regression - Get text node's parent tagName only if it's an Element node (GH-38)"
] = {
  test() {
    const document = parse5.parse('<template>yo<div></div>42</template>')
    const originalGetTagName = (this.originalGetTagName =
      treeAdapters.default.getTagName)
