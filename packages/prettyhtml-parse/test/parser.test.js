...
exports[
  'Regression - HTML5 Legacy Doctype Misparsed with htmlparser2 tree adapter (GH-45)'
] = function() {
  const html =
    '<!DOCTYPE html SYSTEM "about:legacy-compat"><html><head></head><body>Hi there!</body></html>'
  const document = parse5.parse(html, { treeAdapter: treeAdapters.htmlparser2 })

  assert.strictEqual(
    document.childNodes[0].data,
...
exports[
  'Regression - Incorrect arguments fallback for the parser.parseFragment (GH-82, GH-83)'
] = {
  beforeEach() {
...
exports[
  "Regression - Don't inherit from Object when creating collections (GH-119)"
] = {
  beforeEach: function() {
  ...
  test: function() {
    ...
    assert.strictEqual(
      treeAdapters.htmlparser2.getAttrList(fragment.childNodes[0]).length,
      1
    )
  }
}
