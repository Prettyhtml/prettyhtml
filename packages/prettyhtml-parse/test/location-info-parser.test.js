...
generateTestsForEachTreeAdapter(module.exports, (_test, treeAdapter) => {
  _test[
    'Regression - Incorrect LocationInfo.endOffset for implicitly closed <p> element (GH-109)'
  ] = function() {
    ...
    assert.strictEqual(
      html.substring(firstPLocation.startOffset, firstPLocation.endOffset),
      '<p>1'
    )
  }

  _test[
    'Regression - Incorrect LocationInfo.endOffset for element with closing tag (GH-159)'
  ] = function() {
    const html = '<i>1</i>2'
    ...
    assert.strictEqual(
      html.substring(location.startOffset, location.endOffset),
      '<i>1</i>'
    )
  }

  _test[
    'Regression - Location info not exposed with parseFragment (GH-82)'
  ] = function() {
  ...
  _test[
    'Regression - location info mixin error when parsing <template> elements (GH-90)'
  ] = function() {
  ...
  _test[
    'Regression - location info not attached for empty attributes (GH-96)'
  ] = function() {
    ...
    assert.ok(
      treeAdapter.getNodeSourceCodeLocation(firstChild).attrs['test-attr']
    )
  }

  _test[
    'Regression - location line incorrect when a character is unconsumed (GH-151)'
  ] = function() {
    const html = [
      '<html><body><script>',
  ...
  _test[
    'Regression - location.startTag should be available if end tag is missing (GH-181)'
  ] = function() {
    const html = '<p>test'
